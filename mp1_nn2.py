...
def modified_nn(batch_size = 128, kernel_size = 3, pool_size = 2, filters = 16, dropout = 0.2):
    with tensorflow.device('/cpu:0'):
        ...
        # Print and plot model
        model.summary()
        plot_model(model, to_file='mp1_nn2'+str(filters)+'.png', show_shapes=True)
...
def main():
    ...
    modified_nn()
    # FINAL NN
    modified_nn(filters = 256, dropout = 0.45)
    # TESTING CODE BELOW
    #filters_arr = [16,32,64,128,256,512,1024]
    #for filter in filters_arr:
    #    modified_nn(filters = filter, dropout = 0.45)
    return
